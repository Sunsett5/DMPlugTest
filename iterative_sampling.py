...
def init_algo(opt, model, H_funcs=None, sigma_0=0.01, deg=None, betas=None):
    if opt.algo == 'ddnm':
        ...
        gamma = 100
        # print(gamma)
        algo = ReSample(model, H_funcs, sigma_0, gamma=gamma)
    elif opt.algo == 'daps':
...
def prepare_measurement(opt, task_config, device):
    ...
    if 'sr' in deg:
        ...
        if 'box' in deg:
            ...
            # Random inpainting
            missing_r = 3 * torch.randperm(config.data.image_size**2)[:int(config.data.image_size**2 * 0.5)].to(device).long()
...
def sample_image(opt, config=None, model_config=None, device='cuda'):
    ...
    with torch.no_grad():
        for x_orig, classes in pbar:
            ...
            with torch.no_grad():
                skip = (opt.num_timesteps-1) // opt.timesteps
                seq = range(0, opt.num_timesteps, skip)
                ...
                for i, j in tqdm(zip(reversed(seq), reversed(seq_next))):
                    ...
                    if opt.algo == 'reddiff':
                        x0_t, add_up = algo.cal_x0(xt, x0_t_last, t, at, at_next, y_0)
                    else:
                        x0_t, add_up = algo.cal_x0(xt, t, at, at_next, y_0)
            ...
            for j in range(len(x)):
                ...
                if i == len(x)-1 or i == -1:
                    ...
                    avg_psnr += psnr
                    # print(x[j].shape)
                    avg_ssim += ssim(x[j].cpu().numpy(), orig.cpu().numpy(), data_range=x[j].cpu().numpy().max() - x[j].cpu().numpy().min(), channel_axis=0)
